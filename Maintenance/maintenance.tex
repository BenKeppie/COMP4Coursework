...
Text & Holds the text of a QLineEdit whilst the data entry is being validated &
TrickNameExpression & Holds the compiled regular expression for the trick name &
Match & Matches the Text of a QLineEdit against the appropriate regular expression &
sql & holds the sql text for sqlite3 queries &
self.LastMarker & Holds the coordinates of the last marker placed on the google map &
self.html & Holds all of the HTML/Javascript for my google maps object &
var markers & A Javascript list for holding the markers coordinates &
var ContentString & A Javascript variable for holding the text description for each marker &
Send & A variable to compile the SMTP protocol for sending an email &
msg & A variable to format the email to send &
result & Stores the result of an sqlite3 query &
db_name & The name of the programs database &
Choice & Stores the users choice whilst navigation the CLI &
Finished & A while loop condition that changes to True once the conditions are satisfied & 
